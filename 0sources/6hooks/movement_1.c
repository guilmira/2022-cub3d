...
static void abort_crapping(t_prog *game)
{
	...
	{
		if(flag == 1)//right
			game->pl.position_coor[0] -= 0.05;
		else if(flag == 2)//left
			game->pl.position_coor[0] += 0.05;
		else if(flag == 3) //top
			game->pl.position_coor[1] -= 0.05;
	...
	game->pl.position[1] = ceil(game->pl.position_coor[1] / game->map2D.pixel_per_block[1]);
}
...
void filter_final_pos(t_prog *game, double new_pos[], int flag)
{
	if (flag == 0)
	{
		...
		game->pl.position[0] = floor(new_pos[0] / game->map2D.pixel_per_block[0]);
		game->pl.position[1] = game->map2D.map_y - ceil(new_pos[1] / game->map2D.pixel_per_block[1]);
	}
	if (flag == 1 || flag == 2)
	{
		game->pl.position_coor[1] = new_pos[1];
