...
int pixel_per_block[], t_prog *game)
{
	...
	while (++j < pixel_per_block[1])
	{
		...
		line_end[1] = line_start[1];
		if (!j)
			put_line_h(line_start, line_end, GREEN, game);
		else
			put_line_h(line_start, line_end, BLUE, game);
	}
	put_line_h(line_start, line_end, GREEN, game);
	put_line_v(line_start, pixel_per_block[1], GREEN, game);
	put_line_v(line_end, pixel_per_block[1], GREEN, game);
}

/** PURPOSE : Draw all the 2D wall to screen. */
static void	draw_wall_two_dim(t_prog *game)
{
	int		i;
	int		j;
