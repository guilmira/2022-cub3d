...
and RGB values range from 0 to 255, we can perfectly fit a integer (as an int is 4 bytes).  */
...
/* Now, if we introduce trgb_translate(0, 255, 0, 0) looking for a pure red, i get
the value 16711680 which is equivalent to 0x00FF0000 */
...
{
	return (trgb_translate( get_transparent(colour_code) , (255 - get_red(colour_code)) , (255 - get_green(colour_code)), (255 - get_blue(colour_code)) ) );
}